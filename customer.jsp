	...
	//Update command
		if(btnval.equalsIgnoreCase("update"))
		{
			...
			try
			{
				Class.forName("oracle.jdbc.driver.OracleDriver");
				Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "inventory", "inventory");
				PreparedStatement psmt=conn.prepareStatement("update customer set nm=?,address=? where id=?");
				psmt.setString(1,t2);
				psmt.setString(2,t3);
				psmt.setString(3,t1);
				psmt.executeQuery();
		...
		//Record of psearch
					...
					
				{
					...
					try
					{
						...
						Statement smt=conn.createStatement();
						if(!s1.equals("all")){
							ResultSet rs=smt.executeQuery("select * from customer where "+s1+" = '"+t1+"' ");
							%>
							<table border=2>
							<tr>
								<th>Customer ID</th>
							...
							<%
							if(s1.equals("all")){
								ResultSet rs2=smt.executeQuery("select * from customer");
								%>
								<table border=2>
								<tr>
									<th>Customer ID</th>
