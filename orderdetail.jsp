...
<%
	...
	if(btnval.equalsIgnoreCase("save"))
	{
		...
		try
		{
			...
			PreparedStatement psmt=conn.prepareStatement("insert into orderdetail values(?,?,?,?)");
			psmt.setString(1,t1);
			psmt.setString(2,t2);
			psmt.setString(3,t3);
			psmt.setString(4,t4);
	...
	//delete command
			if(btnval.equalsIgnoreCase("delete"))
			{
				...
				try
				{
					Class.forName("oracle.jdbc.driver.OracleDriver");
					Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "inventory", "inventory");
					PreparedStatement psmt=conn.prepareStatement("delete from orderdetail where id=?");
					psmt.setString(1,t1);
					psmt.executeQuery();
			...
			if(btnval.equalsIgnoreCase("update"))
			{
				...
				try
				{
					Class.forName("oracle.jdbc.driver.OracleDriver");
					Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "inventory", "inventory");
					PreparedStatement psmt=conn.prepareStatement("update orderdetail set qty=?,adate1=?,rdate1=? where id=?");
					psmt.setString(1,t2);
					psmt.setString(2,t3);
					psmt.setString(3,t4);
					psmt.setString(4,t1);
			...
			//Record of Allsearch
					if(btnval.equalsIgnoreCase("allsearch"))
					{
						try
						{
							...
							ResultSet rs=smt.executeQuery("select * from Orderdetail");
							%>
							<table border=2>
							<tr>
					...
					if(btnval.equalsIgnoreCase("psearch"))
					{
						...
						try
						{
							...
							Connection conn=DriverManager.getConnection("ojdbc:oracle:thin:@localhost:1521:xe", "inventory", "inventory");
							Statement smt=conn.createStatement();
							ResultSet rs=smt.executeQuery("select * from Orderdetail where id='"+t1+"'");
							%>
							<table border=2>
							<tr>
								<th>Orderdetail ID</th>
